...
// 
func GET__api__org__name__team__alias__members(info *RestRequestInfo, sideEffects *RestSideEffects) (map[string]interface{}, RestError) {
    if info.Account == nil {
    ...
    if !ok || err != nil {
        return nil, UnauthorizedError("Must be member of org to see team members").Log()
    }

    // Look up team
    team, err := org.Team(info.URLVars["alias"])
    if err != nil {
        // TODO: determine actual cause of error
        return nil, URLNotFoundError().Log()
    }
...
// Add/Remoe members
func POST__api__org__name__team__alias__members(info *RestRequestInfo, sideEffects *RestSideEffects) (map[string]interface{}, RestError) {
    if info.Account == nil {
        return nil, NotLoggedInError().Log()
    }
    ...
    if !ok || err != nil {
        return nil, UnauthorizedError("Must be owner of org to change team members").Log()
    }
